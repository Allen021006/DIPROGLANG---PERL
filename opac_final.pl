...
my %CONFIG = (
    ...
    app_name    => "Perlfect Library: An OPAC System",
    version     => "1.0.0",
...
sub splash_screen {
    ...
    print "\e[0m\n";
    print "\e[96m      Perlfect Library: An OPAC System\e[0m\n";
...
sub add_book {
    ...
    # Added ddc format validation -tyrone
    unless ($dewey =~ /^\d{3}(\.\d{1,4})?$/){
        print "\n\e[91m  ERROR: Dewey Decimal must follow the proper format.\e[0m\n";
        pause();
        return;
    }
    ...
    unless ($year =~ /^\d{4}$/ && $year >= 1950 && $year <= $current_year) { # updated to limit the year from 1950 to present -tyrone
        print "\n\e[91m  ERROR: Year must be a plausible year.\e[0m\n";
        pause();
        return;
    }
...
sub view_books {
    ...
    print "  \e[33m[1]\e[0m Sort by ID\n";
    print "  \e[33m[2]\e[0m Sort by Title\n";
    print "  \e[33m[3]\e[0m Sort by Author\n";
    print "  \e[33m[4]\e[0m Sort by Year\n";
    print "  \e[33m[5]\e[0m Sort by Dewey (Default)\n\n";

    my $choice = prompt("Choose sorting method", "5");
    ...
    if ($choice eq '1') {
        ...
        $order = "dewey ASC, title ASC";
    }

    my $sth = $dbh->prepare("SELECT * FROM books ORDER BY $order");
    $sth->execute();
...
sub search_books {
    ...
    unless ($mode =~ /^[123]$/) {
        print "\n\e[91m  ERROR: Invalid search type. Please choose 1, 2, or 3.\e[0m\n";
        pause();
        return;
    }
    ...
    if ($mode eq '1') {
        ...
        if ($keyword =~ /^(\d+)00$/ || $keyword eq '0') {
            my $base = $1 // 0;
            my $low  = $base * 100;        
            my $high = $low + 99;          
            my $sth  = $dbh->prepare(
                "SELECT * FROM books WHERE CAST(dewey AS DECIMAL(10,4)) BETWEEN ? AND ? ORDER BY dewey ASC"
            );
            $sth->execute($low, $high);
            
            cls();
            banner("SEARCH RESULTS: DDC $low - $high");
            print "\n";
            my $count = 0;
            while (my @row = $sth->fetchrow_array()) {
                my $book = Book->new(
...
sub edit_book {
    ...
    # Show catalog for reference
    my $sth_all = $dbh->prepare("SELECT * FROM books ORDER BY id ASC");
    $sth_all->execute();
    ...
    $sth_all->finish(); undef $sth_all;

    if (!$has_books) {
        print "  \e[90m(No books in catalog)\e[0m\n";
        pause(); return;
    }
    ...
    # Validation
    unless ($dewey eq $book->{dewey} || $dewey =~ /^\d{3}(\.\d{1,4})?$/) {
        print "\n\e[91m  ERROR: Dewey Decimal must follow the proper format.\e[0m\n";
        pause(); return;
    }
    ...
    unless ($year eq $book->{year} || $year =~ /^\d{4}$/ && $year >= 1950 && $year <= $current_year) {
        print "\n\e[91m  ERROR: Year must be a plausible year.\e[0m\n";
        pause(); return;
    }
...
sub borrow_book {
    ...
    # Date validation (maximum three weeks of borrow time)
    my ($yb, $mb, $db) = split /-/, $borrow_date;

    my $borrow_period = timelocal(0,0,0,$db, $mb-1, $yb-1900);
    my $max_period = $borrow_period + (21 * 24 * 60 * 60);
    my ($sec, $min, $hour, $day, $mon, $year) = localtime($max_period);
    $year += 1900;
    $mon += 1;

    my $max_date = sprintf("%04d-%02d-%02d", $year, $mon, $day);

    if ($due_date gt $max_date){
        print "\n\e[91m  Books can only be borrowed for three weeks maximum.\e[0m\n";
        pause(); return;
    }
...
sub return_book {
    ...
    # Show only borrowed books for reference
    my $sth_all = $dbh->prepare("SELECT * FROM books WHERE status='borrowed' ORDER BY id ASC");
    $sth_all->execute();
    ...
    if (!$has_books) {
        print "  \e[90m(No books are currently borrowed)\e[0m\n";
        pause(); return;
    }
...
sub view_borrow_records {
    ...
    while (my @r = $sth->fetchrow_array()) {
        ...
        if ($rdate) {
            ...
            my $days_late = compute_days_late($ddate, $today);
            my $fee       = $days_late * $CONFIG{fee_per_day};
            $status_str   = "OVERDUE";
            $status_color = "\e[91m";
            $overdue_count++;
    ...
    # Overdue breakdown section
    if (@overdue_list) {
        print "\n";
        banner("OVERDUE FEES BREAKDOWN");
        print "\n";
        printf("  \e[1m%-4s  %-28s  %-18s  %-12s  %-9s  %s\e[0m\n",
            "ID", "TITLE", "PATRON", "DUE DATE", "DAYS LATE", "FEE (PHP)");
        divider();

        for my $rec (@overdue_list) {
            printf("  \e[91m%-4d  %-28s  %-18s  %-12s  %-9d  PHP %.2f\e[0m\n",
                $rec->{borrow_id},
                substr($rec->{title},  0, 27),
                substr($rec->{patron}, 0, 17),
                $rec->{due_date},
                $rec->{days_late},
                $rec->{fee});
        }
        ...
        divider();
        printf("  \e[1;91m%-74s PHP %.2f\e[0m\n", "TOTAL OUTSTANDING FEES:", $total_fees);
    }
...
sub about_screen {
    cls();
    banner("ABOUT PERLFECT LIBRARY & LANGUAGE PARADIGMS");
    print "\n";
...
END {
    ...
    cls();
    print "\e[1;33m\n  Thank you for using Perlfect Library!\e[0m\n";
